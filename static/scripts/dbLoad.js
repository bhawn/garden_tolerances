	...
	let entry = `
			<div class="card w-100 border-0 position-relative" style="overflow:hidden;background-color:black;margin-top: 16px;">
				...
				<a class="carousel-control-prev carousel-control" href="#id_{{id}}" role="button" data-slide="prev" style='z-index:1;'>
					<span class="carousel-control-prev-icon" aria-hidden="true"></span>
					<span class="sr-only">Previous</span>
				</a>
				<a class="carousel-control-next carousel-control" href="#id_{{id}}" role="button" data-slide="next" style='z-index:1;'>
	...
	function makeEntry(data) { 												
		let entryStuff = entryInfo(data);
		if(entryStuff === -1) return;
		let entryJq = $(entryStuff);
	...
	function parseEntries(data) {
		csvJSON(data).sort(function(a,b){
	...
	function csvJSON(csv) {
		let lines = csv.replaceAll("\r","").split("\n"),
			result = [],
			headers = lines[0].replaceAll("\"","").split(",");
	...
	$(function() {
		...
		const numberFilter = function(filterValue, objvalue, orig_string) {
			if(typeof filterValue !== 'string')
				return true;
			let config = {};
			let equality = filterValue.indexOf("=") != -1;
			let intvalue = format(filterValue);
			if(intvalue === "") return "";
			let compare;
		...
		$('#search').on( "keyup", function() {
			// Declare variables
			let filter,  li, a, i, txtValue, num;
			...
			for(i = 0; i < li.length; i++) {
				...
				if(txtValue.indexOf(filter) > -1 || num.length && numberFilter(filter, num, num) || fil3) {
					li[i].style.display = "";
				} else {
